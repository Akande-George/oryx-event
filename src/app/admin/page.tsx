...
function AdminDashboardContent() {
  ...
  const [selectedAttendee, setSelectedAttendee] = useState<
    ReturnType<typeof buildAttendees>[number] | null
  >(null);
  ...
  useEffect(() => {
    ...
    async function loadAdminData() {
      const [eventsRes, packagesRes, ordersRes, hotelsRes, roomsRes, bookingsRes] =
        await Promise.all([
          supabase.from("events").select("*").order("created_at", { ascending: false }),
          supabase.from("ticket_packages").select("*").order("created_at", { ascending: false }),
      ...
      if (packagesRes.data) {
        const groupedPackages = (packagesRes.data as TicketPackage[]).reduce(
          (acc, pkg) => {
            const list = acc[pkg.event_id] ?? [];
            acc[pkg.event_id] = [...list, pkg];
            return acc;
          },
      ...
      if (roomsRes.data) {
        const groupedRooms = (roomsRes.data as RoomType[]).reduce(
          (acc, room) => {
            const list = acc[room.hotel_id] ?? [];
            acc[room.hotel_id] = [...list, room];
            return acc;
          },
          {} as Record<string, RoomType[]>,
        );
        setHotelRooms(groupedRooms);
      }
    }
  ...
  const handleConfirmOrder = (id: string) => {
    ...
    if (selectedOrder?.id === id)
      setSelectedOrder((o) => (o ? { ...o, status: "confirmed" } : o));
  };
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          <SelectContent>
                            {(["Regular", "VIP", "Table"] as const).map((t) => (
                                ...
                                >
                                  ...
                                  <DialogContent>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                      ...
                                      <strong>{event.title}</strong>? This
                      ...
                      <div className="overflow-x-auto mt-3">
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {pkgs.map((pkg) => {
                              const sold =
                                pkg.total_slots - pkg.available_slots;
                              return (
                                <tr
                                  key={pkg.id}
                                  className="border-b border-border/40 hover:bg-muted/20 transition-colors"
                      ...
                      <div className="space-y-4 py-1">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          <div className="col-span-2">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {selectedAttendee.event?.venue} ·{" "}
                              {selectedAttendee.event
                                ? formatDate(selectedAttendee.event.date)
                                : ""}
                            </p>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <p className="font-heading font-bold">
                              {formatPrice(selectedAttendee.total_price)}
                            </p>
                          </div>
                          <div className="col-span-2">
                            <p className="text-xs text-muted-foreground mb-0.5">
                          ...
                          {selectedAttendee.pkg?.perks &&
                            selectedAttendee.pkg.perks.length > 0 && (
                              <div className="col-span-2">
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={() =>
                              window.open(
                                `mailto:${selectedAttendee.guest_email}?subject=Your ticket for ${selectedAttendee.event?.title ?? "the event"}`,
                              )
                            }
                          >
                            ...
                            >
                              ...
                              <td className="px-5 py-4">
                                <div className="flex items-center gap-1">
                                  ...
                                  <Dialog
                                    open={deleteHotelId === hotel.id}
                                    onOpenChange={(open) =>
                                      !open && setDeleteHotelId(null)
                                    }
                                  >
                                    <DialogTrigger
                                      render={
                                    ...
                                    <DialogContent>
                                      ...
                                      <p className="text-sm text-muted-foreground">
                                        Are you sure you want to delete{" "}
                                        <strong>{hotel.name}</strong> and its
                                        rooms? This action cannot be undone.
                                      </p>
                                      <DialogFooter className="mt-4">
                                        <Button
                                          variant="outline"
                      ...
                      <tbody>
                        {bookings.map((booking) => {
                          const hotel = hotels.find(
                            (h) => h.id === booking.hotel_id,
                          );
                          const room = (
                            hotelRooms[booking.hotel_id] ?? []
                          ).find((r) => r.id === booking.room_type_id);
                          return (
                            <tr
                              key={booking.id}
                              className="border-b border-border/40 hover:bg-muted/20 transition-colors cursor-pointer"
                ...
                >
                  <DialogContent className="max-w-md">
                    ...
                    {selectedBooking &&
                      (() => {
                        const hotel = hotels.find(
                          (h) => h.id === selectedBooking.hotel_id,
                        );
                        const room = (
                          hotelRooms[selectedBooking.hotel_id] ?? []
                        ).find((r) => r.id === selectedBooking.room_type_id);
                        return (
                          <div className="space-y-4 py-1">
                            <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                              <div className="w-10 h-10 rounded-full gradient-primary flex items-center justify-center text-white text-sm font-bold shrink-0">
                            ...
                            <div className="grid grid-cols-2 gap-3 text-sm">
                              ...
                              <div className="col-span-2">
                                ...
                                <p className="font-heading font-bold text-foreground">
                                  {formatPrice(selectedBooking.estimated_total)}
                                </p>
                              </div>
                            ...
                            <Button
                              ...
                              onClick={() =>
                                window.open(
                                  `mailto:${selectedBooking.guest_email}?subject=Your booking request for ${hotel?.name ?? "your stay"}`,
                                )
                              }
                            >
