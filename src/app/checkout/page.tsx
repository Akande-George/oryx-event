...
function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    async function loadCheckoutData() {
      ...
      const [{ data: liveEvent }, { data: livePackage }, { data: authUser }] =
        await Promise.all([
      ...
      if (liveEvent) setEvent(liveEvent as Event);
      if (livePackage) setPkg(livePackage as TicketPackage);
  ...
  const handlePayment = async () => {
    setLoading(true);
    if (!event || !pkg) {
      toast.error("This ticket package is no longer available.");
      setLoading(false);
      return;
    }
            ...
            <div key={s} className="flex items-center gap-3">
              {i > 0 && (
                <div
                  className={cn(
                    "h-px flex-1 w-12",
                    step === "payment" ? "bg-primary" : "bg-border/50",
                  )}
                />
              )}
              <div
